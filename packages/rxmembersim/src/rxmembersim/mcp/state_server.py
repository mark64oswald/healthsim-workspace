"""MCP Server for RxMemberSim state management capabilities.
...
from rxmembersim.mcp.session import RxMemberSessionManager

# Configure logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("rxmembersim.mcp.state")

# Initialize MCP server
app = Server("rxmembersim-state")
...
def format_cohort_saved(cohort: Any) -> str:
    ...
    lines = [
        ...
        f"- Rx Members: {cohort.get_entity_count('rx_members')}",
...
def format_cohort_loaded(cohort: Any, summary: dict) -> str:
    ...
    lines = [
        ...
        "",
        f"- Rx Members loaded: {summary['rx_members_loaded']}",
        f"- Total entities: {summary['total_entities']}",
    ]

    if summary.get("rx_members_skipped", 0) > 0:
        lines.append(f"- Rx Members skipped (conflicts): {summary['rx_members_skipped']}")
...
def format_cohort_list(cohorts: list[dict]) -> str:
    ...
    if not cohorts:
        return (
            ...
            "Generate some rx members and use `save_cohort` to save your work."
        )
    ...
    for s in cohorts:
        ...
        rx_member_count = s["rx_member_count"]
        tags = ", ".join(s.get("tags", [])) if s.get("tags") else ""

        line = f"- **{name}** ({created}) - {rx_member_count} rx members"
        if tags:
            line += f" [tags: {tags}]"
...
def format_cohort_deleted(info: dict) -> str:
    ...
    return f"**Deleted:** \"{info['name']}\"\n- {info['rx_member_count']} rx members removed"
    ...
    return [
        Tool(
            ...
            description=(
                ...
                "Captures all rx members and pharmacy data with provenance tracking."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "name": {
                        "type": "string",
                        "description": "Name for the cohort (e.g., 'pbm-testing', 'specialty-demo')",
                        "minLength": 1,
                    },
                    "description": {
                        "type": "string",
                    ...
                    "tags": {
                        "type": "array",
                        "description": "Optional tags for organization (e.g., ['testing', 'specialty'])",
                        "items": {"type": "string"},
                    },
                    "member_ids": {
                        "type": "array",
                        "description": "Specific member IDs to save (default: all rx members)",
                        "items": {"type": "string"},
                    },
                },
                "required": ["name"],
        ...
        Tool(
            ...
            description=(
                ...
                "Can replace or merge with existing rx members."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    ...
                    "member_ids": {
                        "type": "array",
                        "description": "Specific member IDs to load (default: all rx members)",
                        "items": {"type": "string"},
                    },
                },
            },
        ...
        Tool(
            ...
            description=(
                ...
                "rx member count and provenance breakdown."
            ),
            inputSchema={
                "type": "object",
                "properties": {},
    ...
    if session_manager.count() == 0:
        return [
            TextContent(
                type="text",
                text=format_error(
                    "Nothing to save. Generate some rx members first with `generate_rx_member`."
                ),
            )
        ]
...
async def handle_load_cohort(arguments: dict) -> list[TextContent]:
    ...
    try:
        ...
        if mode == "replace" and current_count > 0:
            result = f"_Replaced {current_count} existing rx members._\n\n" + result
...
async def handle_workspace_summary(_arguments: dict) -> list[TextContent]:
    ...
    if summary["rx_member_count"] == 0:
        return [
            TextContent(
                type="text",
                text=(
                    "**Workspace is empty.**\n\n"
                    "Generate rx members with `generate_rx_member` or "
                    "load a cohort with `load_cohort`."
                ),
            )
        ]
    ...
    lines = [
        "**Current Workspace:**",
        "",
        f"- Rx Members: {summary['rx_member_count']}",
    ]

    if summary["prescription_count"] > 0:
        lines.append(f"- Prescriptions: {summary['prescription_count']}")
    if summary["pharmacy_claims_count"] > 0:
        lines.append(f"- Pharmacy Claims: {summary['pharmacy_claims_count']}")
    if summary["dur_alerts_count"] > 0:
        lines.append(f"- DUR Alerts: {summary['dur_alerts_count']}")
    if summary["prior_auth_count"] > 0:
        lines.append(f"- Prior Authorizations: {summary['prior_auth_count']}")
    ...
    if prov:
        ...
        if prov.get("generated", 0) > 0:
            lines.append(f"- Generated: {prov['generated']} rx members")
        if prov.get("loaded", 0) > 0:
            lines.append(f"- Loaded: {prov['loaded']} rx members")
        if prov.get("derived", 0) > 0:
            lines.append(f"- Derived: {prov['derived']} rx members")

    return [TextContent(type="text", text="\n".join(lines))]


def get_session_manager() -> RxMemberSessionManager:
    """Get the session manager instance for external use."""
    return session_manager


def set_session_manager(manager: RxMemberSessionManager) -> None:
...
async def main():
    ...
    logger.info("Starting RxMemberSim State Management MCP Server")
